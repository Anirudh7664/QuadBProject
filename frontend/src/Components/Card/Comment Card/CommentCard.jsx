...
const CommentCard = ({ userReview, setReviews, reviews, fetchReviews }) => {
    ...
    const getUser = async () => {
        const { data } = await axios.get(`http://localhost:8000/api/auth/getuser`, {
            headers: {
    ...
    const handleDeleteComment = async () => {
        try {
            const { data } = await axios.delete(`http://localhost:8000/api/review/deletereview/${userReview._id}`, {
                headers: {
                    'Authorization': authToken
                }
            })
    ...
    const deleteCommentByAdmin = async () => {
        if (authUser) {
            try {
                const { data } = await axios.delete(`http://localhost:8000/api/admin/review/${userReview._id}`,
                    {
                        headers: {
                            'Authorization': authToken
                        }
    ...
    const sendEditResponse = async () => {
        if (!editComment && !value) {
            ...
            try {
                if (authToken) {
                    const response = await axios.put(`http://localhost:8000/api/review/editreview`,
                    ...
                    <Box sx={{ height: 20, transform: 'translateZ(0px)', flexGrow: 1 }}>
                        ...
                        >
                            ...
                            <SpeedDialAction
                                icon={<AiFillDelete />}
                                tooltipTitle={"Delete"}
                                onClick={authUser ? deleteCommentByAdmin : handleDeleteComment}
                            />
