...
const SingleProduct = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!productFilter.includes(productInfo.category)) {
            ...
            setError(false)
            try {
                const { data } = await axios.put(`http://localhost:8000/api/product/fetchproduct/${product._id}`, { productDetails: productInfo }, {
                    headers: {
    ...
    const deleteProduct = async () => {
        try {
            const { data } = await axios.delete(`http://localhost:8000/api/product/fetchproduct/${product._id}`, {
                headers: {
                    'Authorization': authToken
                }
            });
