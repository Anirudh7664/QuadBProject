...
const UserWishlistItem = ({ authToken, id, commonGetRequest }) => {
    ...
    useEffect(() => {
        commonGetRequest(`http://localhost:8000/api/admin/getwishlist`, id, setUserWishlist);
    }, [])
    const removeCartItemByAdmin = async (product) => {
        try {
            console.log(product._id);
            const { data } = await axios.delete(`http://localhost:8000/api/admin/userwishlist/${product._id}`, {
                headers: {
