...
const UserCartItem = ({ commonGetRequest, id, authToken }) => {
    ...
    const removeCartItemByAdmin = async (product) => {
        try {
            const { data } = await axios.delete(`http://localhost:8000/api/admin/deleteproduct/${product._id}`, {
                headers: {
    ...
    useEffect(() => {
        commonGetRequest(`http://localhost:8000/api/admin/getcart`, id, setUserCart);
    }, [])
    return (
        <>
