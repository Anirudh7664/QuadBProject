...
const getCart = async (setProceed, setCart, authToken) => {
    if (setProceed) {
        const { data } = await axios.get(`http://localhost:8000/api/cart/fetchcart`,
            {
                headers: {
...
const getWishList = async (setProceed, setWishlistData, authToken) => {
    if (setProceed) {
        const { data } = await axios.get(`http://localhost:8000/api/wishlist/fetchwishlist`,
            {
                headers: {
                    'Authorization': authToken
                }
...
const getAllProducts = async (setData) => {
    try {
        const { data } = await axios.get(`http://localhost:8000/api/product/fetchproduct`);
        setData(data)
...
const getSingleProduct = async (setProduct, id, setLoading) => {
    ...
    const { data } = await axios.get(`http://localhost:8000/api/product/fetchproduct/${id}`)
    setProduct(data)
    setLoading(false);
