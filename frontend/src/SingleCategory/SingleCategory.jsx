...
const SingleCategory = () => {
    ...
    if (cat === 'book') {
        productFilter.push('All','Price Low To High', 'Price High To Low')
    }
    else if (cat === 'cloths') {
        productFilter.push('All','Price Low To High', 'Price High To Low')
    }
    ...
    const getData = async () => {
        ...
        const { data } = await axios.post(`http://localhost:8000/api/product/fetchproduct/category`, { userType: cat, userCategory: filter })
        setProductData(data)
        setIsLoading(false)
    }
    useEffect(() => {
