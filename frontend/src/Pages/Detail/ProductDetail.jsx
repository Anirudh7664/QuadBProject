...
const ProductDetail = () => {
    ...
    const addToCart = async (product) => {
        if (setProceed) {
            try {
                ...
                const { data } = await axios.post(`http://localhost:8000/api/cart/addcart`, { _id: product._id, quantity: productQuantity }, {
                    headers: {
    ...
    const addToWhishList = async (product) => {
        if (setProceed) {
            try {
                const { data } = await axios.post(`http://localhost:8000/api/wishlist/addwishlist`, { _id: product._id }, {
                    headers: {
                        'Authorization': authToken
                    }
                })
    ...
    const getSimilarProducts = async () => {
        const { data } = await axios.post(`http://localhost:8000/api/product/fetchproduct/type`, { userType: cat })
