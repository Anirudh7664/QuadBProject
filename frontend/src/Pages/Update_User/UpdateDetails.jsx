...
const UpdateDetails = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!userDetails.email && !userDetails.firstName && !userDetails.phoneNumber && !userDetails.lastName && !userDetails.address && !userDetails.city && !userDetails.userState && !userDetails.zipCode) {
                ...
                const { data } = await axios.put(`http://localhost:8000/api/auth/updateuser`, {
    ...
    const handleResetPassword = async (e) => {
        ...
        try {
            if (!password.currentPassword && !password.newPassword) {
                ...
                const { data } = await axios.post(`http://localhost:8000/api/password/reset/password`, {
                    id: userData._id,
    ...
    const deleteAccount = async () => {
        try {
            const deleteUser = await axios.delete(`http://localhost:8000/api/auth/delete/user/${userData._id}`, {
                headers: {
                    'Authorization': authToken
                }
            });
